...
class RBMCell(object):
    ...
    def __call__(self, inputs, state, hidden_dim, idx, scope=None, reuse=False):
        with tf.variable_scope(scope or type(self).__name__, reuse=reuse):
            ...
            T = _conv_linear([_o, c], self.filter_size, hidden_dim, True,
                             scope='convT_' + str(idx), reuse=reuse)
            R = _conv_linear([o], self.filter_size, hidden_dim, True,
                             scope='convR_' + str(idx), reuse=reuse)
...
class RBM(object):
    ...
    def __init__(self, shape, filter_size, num_layers, hidden_dims, jump_stride,
                 p_TD):
    ...
    def __call__(self, inputs, state_list, scope=None, reuse=False):
        with tf.variable_scope(scope or type(self).__name__, reuse=reuse):
            ...
            for idx, layer_hiddim in enumerate(self.hidden_dims):
                o, c = self.rbmcell(_inputs[-1], state_list[idx], layer_hiddim,
                                    idx)
...
def _conv_linear(args, filter_size, num_features, bias,
    ...
    with tf.variable_scope(scope or "Conv", reuse=reuse):
        ...
        if len(args) == 1:
            res = tf.nn.conv2d(args[0], matrix, strides=[1, 1, 1, 1],
                               padding='SAME')
        else:
