...
class MCNET(object):
    ...
    def build_model(self):
        self.diff_in = tf.placeholder(tf.float32, self.diff_shape,
                                      name='diff_in')
        self.xt = tf.placeholder(tf.float32, self.xt_shape, name='xt')
        self.target = tf.placeholder(tf.float32, self.target_shape,
                                     name='target')

        cell = RBM([self.image_size[0] / 8, self.image_size[1] / 8], [3, 3], 15,
                   self.hidden_dims, 5, 0.5)
